...
const hearingTestContainer = document.getElementById("hearing-test-container");
...
function validateForm() {
  const selectedYear = document.getElementById("year-of-birth").value !== "";
  const otherInput = document.getElementById("other-text");
  const atLeastOneRadioChecked = Array.from(
    document.querySelectorAll(".input-radio")
  ).some((radioButton) => radioButton.checked);
  const atLeastOneCheckboxChecked = Array.from(
    document.querySelectorAll(".input-checkbox")
  ).some((checkbox) => checkbox.checked);
  const otherCheckboxChecked =
    document.getElementById("other-checkbox").checked;
  const otherInputValid =
    !otherCheckboxChecked ||
    (otherCheckboxChecked && otherInput.value.length >= 5);
  ...
  if (
    ...
    $("#form-btn").addEventListener("click", navigate);
  }
}
...
function getWelcomeSlide() {
  hearingTestContainer.innerHTML = `<div class="hero-image" alt="Hero Image"></div>
    ...
    <section class="part-1">
        ...
        <section>
            ...
            <ul class="radio-list inter-fs-1p5-fw-400-lh-2p25" style="align-self: flex-start;">
                <li class="radio-item">
                    <input type="radio" id="male" name="gender" class="input-radio">
                    <label for="male">Male</label>
                </li>
                <li class="radio-item">
                    <input type="radio" id="female" name="gender" class="input-radio">
                    <label for="female">Female</label>
                </li>
                <li class="radio-item">
                    <input type="radio" id="no-answer" name="gender" class="input-radio">
...
<section class="part-1">
    ...
    <ul class="checkbox-list inter-fs-1p5-fw-400-lh-2p25">
        <li>
            <input type="checkbox" id="noisy-environment" class="input-checkbox">
            <label for="noisy-environment">Noisy environment (restaurants etc.)</label>
        </li>
        <li>
            <input type="checkbox" id="tv-radio" class="input-checkbox">
            <label for="tv-radio">Watching TV/Radio</label>
        </li>
        <li>
            <input type="checkbox" id="talking-to-people" class="input-checkbox">
            <label for="talking-to-people">Talking to people</label>
        </li>
        <li>
            <input type="checkbox" id="other-checkbox" class="input-checkbox">
        ...
        <li>
            <input type="checkbox" id="never" class="input-checkbox">
...
function playback(event, volume = 7) {
  ...
  const audio = new Audio(
    `../../resources/sounds/testsounds/${soundToGet}.ogg`
  );
...
const withHeadphonesText =
  "For both left and right ear, there are 5 sliders, each playing the same sound at a different frequency.<br><br>Adjust each slider to increase or decrease the volume level until you can barely hear the sound on that slider.";
const withoutHeadphonesText =
  "There are 5 sliders, each playing the same sound at a different frequency.<br><br>Adjust each slider up or down to increase or decrease the volume level until you can barely hear the sound on that slider.";
...
function getSoundTestSlide() {
  hearingTestContainer.innerHTML = `<div id="slide-4" class="slide">
          ...
          <section class="test-container">
              ...
              <div class="test-board">
                  ...
                  <div class="volume">
                      ...
                      <div class="nav-container">
                          <section class="nav-button">
                              <div id="previous-sound" data-direction="-1">
                                  <p>
                                      Previous sound
                                  </p>
                          ...
                          <section class="nav-button next-sound">
                              <div id="next-sound" data-direction="4">
                                  <p>
                                      Next sound
                                  </p>
                                  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"
...
$all('.sound-track-button').forEach(button => button.addEventListener('click', handleVolumeButtonClick));
...
nextSoundButton.addEventListener('click', () => {
    ...
    if (iteration < 5) {
        ...
        if (iteration === 5) {
          nextSoundButton.removeEventListener('click', handleVolumeButtonClick);
            nextSoundButton.querySelector('p').textContent = 'Finish test';
            nextSoundButton.addEventListener('click', navigate);
        }
    }
...
previousSoundButton.addEventListener('click', () => {
  if(nextSoundButton.querySelector('p').textContent === 'Finish test'){
    nextSoundButton.querySelector('p').textContent = 'Next sound';
    nextSoundButton.removeEventListener('click', navigate);
    nextSoundButton.addEventListener('click', handleVolumeButtonClick)
  }
...
function handleVolumeButtonClick(event) {
  ...
  if (value + soundTrackEclipseTracker >= 0 && value + soundTrackEclipseTracker <= 5) {
...
function getResultSlide(){
  hearingTestContainer.innerHTML = `<div id="slide-5">
...
document.querySelector('.get-hearing-buds').addEventListener('click', () => {
  window.location.href = "https://www.doro.com/sv-se/shop/smart-devices/hearingbuds/doro-hearingbuds/";
});
document.getElementById('restart-test').addEventListener('click', navigate);
}